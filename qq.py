...
def imbue_with_system_info(prompt: str, system_info: Dict[str, str]) -> str:
    for key, value in system_info.items():
        prompt = prompt.replace(f":r:{key}", str(value))
    return prompt


def explain(llm: Callable, query: str, debug: bool) -> None:
...
2. Format the command similar to how a shell formatter would, with the main command at the beginning of the line and subsequent parts indented based on their role.
3. Explain each part on a new line after its appearance in the formatted command.
4. Separate the command part from its explanation with ' - ' (space, hyphen, space).
5. Use subtle color codes to enhance readability. Apply different colors to commands, options, arguments, and other syntactic structures as appropriate. Be consistent but maintain clarity. Make sure the coloring is added correctly for being streamed to the output terminal.
6. For each part, describe its function, any options or flags used, and their effects.
7. If applicable, mention any potential side effects or important considerations.
8. Conclude with a one-line summary of the overall operation.
9. Use technical terminology where appropriate, but ensure explanations are accessible to users with varying levels of expertise.
...
Input: http -a username:password POST https://api.example.com/v1/users name=John age:=30 roles:='["admin", "user"]'
Output:
...
"""
    system_prompt = imbue_with_system_info(
        system_prompt, {"color_support": color_support}
    )

    if debug:
        print("\033[1;31mDEBUG: explain system prompt\033[0m")
        print(system_prompt)
...
def generate(llm: Callable, query: str, include_environment: bool, debug: bool) -> None:
    color_support = supports_color()
    system_info = {
        "os": platform.system(),
...
"""
    ...
    system_prompt_with_info = imbue_with_system_info(system_prompt, system_info)
    if debug:
        print(f"generate system prompt\n{system_prompt_with_info}")

    messages = [
        {"role": "system", "content": system_prompt_with_info},
    ]
...
def main():
    ...
    parser.add_argument(
        "--no-env",
        action="store_true",
        help="Don't include environment information in command generation",
    )
    parser.add_argument(
        "--debug",
        action="store_true",
    ...
    try:
        ...
        print(
            "Error: litellm is not installed. Please install it with 'pip install litellm'"
        )
        sys.exit(1)
    ...
    try:
        if args.generate:
            generate(llm, query, not args.no_env, args.debug)
        else:
            explain(llm, query, args.debug)
